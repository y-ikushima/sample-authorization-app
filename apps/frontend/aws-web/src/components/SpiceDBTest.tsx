...
import { checkPermission } from "@/lib/spicedb";
...
const SpiceDBTest = () => {
  ...
  const { isGlobalAdmin, loading: globalLoading } =
    useGlobalAdminPermission("spicedb");
  ...
  return (
    ...
    >
      ...
      >
        <small>
          <strong>SpiceDB Test Info:</strong>
          <br />
          • このテストはSpiceDBサービス (http://spicedb-server:8082)
          との接続を確認します
          <br />
          • 各種権限チェック（システム、AWS、グローバル）をテストします
          <br />• SpiceDBの関係ベース認可モデルをテストします
        </small>
      </div>
    </div>
  );
