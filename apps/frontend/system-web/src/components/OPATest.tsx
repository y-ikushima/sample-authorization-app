import { useOPAGlobalPermission } from "@/hooks/useOPAAccessControl";
...
const OPATest = () => {
  ...
  const { isGlobalAdmin, loading: globalLoading } = useOPAGlobalPermission();
    ...
    try {
      ...
      try {
        const customResult = await evaluateOPAQuery(
          "data.authz.user_system_roles",
          { subject: userId }
        );
        addResult(
          "Custom Query (User System Roles)",
  ...
  const testSpecificPermission = async () => {
    // システム管理者権限のテスト
    const systemTests = [
      { resource: "system:system1", permission: "read" },
      { resource: "system:system1", permission: "write" },
      { resource: "system:system1", permission: "delete" },
      { resource: "system:system1", permission: "manage_members" },
      { resource: "system:system2", permission: "read" },
      { resource: "system:system3", permission: "read" },
      { resource: "system:system3", permission: "write" },
    ];
